...
| Closed design language: declared role contracts, closed Tailwind namespaces, generated style symbols (ADR-034, Proposed) | E29 (declare, never infer), E33–E35 (namespace closure is Tailwind's supported mechanism; the numeric spacing scale is a multiplier, not a list; `@source not` bounds detection), E32 (Figma scopes exist and are advisory), E37 (the colour-role grammar and the pairing law the roots follow since 2026-09-04), E38 (the type scale eight of nine type roles sit on; subheading is its emphasized set). E39 (tracking as a function of size, from the system that sets Plex Sans in production). E40 (the outlined text field: `primary` and `error` drawn as lines, the `accent` kind). E41 (Carbon's `$focus` equals `$interactive`: the focus ring aliases primary). **What it does not prove:** that these 20 roots are the right rungs, that the dark surfaces are the right steps, or that closing spacing is right here — only `color-pairs.test.ts`, the distinctness check and the gallery proof support that, on this tree | S/V | `tokens.test.ts` contract refusals and `design-system-classes.test.ts` closure checks, written RED-first, green 2026-09-03 | PROPOSED |
...
| E37 | [Material Design 3 — Color roles](https://m3.material.io/styles/color/roles) | 2026-09-04 | S | 26 roles in six groups built from five words (surface, primary/secondary/tertiary, container, on, variant); "apply colors only in the intended pairs or layering orders"; outline for important boundaries, outline variant for dividers and card edges; add-ons "you probably shouldn't" | ADAPT: the roots renamed and merged to the grammar, the pairing law made mechanical in `COLOR_PAIRS`; hover/pressed kept as fills, status containers as custom colours (ADR-034 fourth pass, `07cf465`) |
| E38 | [Material Design 3 — Type scale & tokens](https://m3.material.io/styles/typography/type-scale-tokens) and Google's generated web token file [material-web `tokens/versions/v0_192/_md-sys-typescale.scss`](https://github.com/material-components/material-web/blob/main/tokens/versions/v0_192/_md-sys-typescale.scss) | 2026-09-04 | S (page) / V (file) | Fifteen baseline styles in five families at three steps, a brand and a plain typeface, the emphasized set, a Major Second scale anchored at 14; the file carries the web metrics the page's token viewer renders in a widget (e.g. body-large 1rem/1.5rem regular, label-large 0.875rem/1.25rem medium, headline-small 1.5rem/2rem regular) | ADAPT: eight of nine roles carry a style to the pixel after heading and display moved onto title-large and headline-large the same day; weight is the recorded deviation (ADR-034 fourth pass; `M3_TYPE_STYLES`) |
| E39 | [Carbon Design System — `@carbon/type` styles](https://github.com/carbon-design-system/carbon/blob/main/packages/type/src/styles.ts) | 2026-09-04 | P | IBM's own type styles for IBM Plex Sans: letterSpacing is a function of size -- 12px styles (caption01, label01, helperText01) +0.32px, 14px styles (label02, bodyShort01, bodyLong01, heading01) +0.16px, 16px and larger 0; the same at regular and semibold | ADOPT as the tracking rule, in em: `tracking.wide` 0.0267em for 12px, `tracking.open` 0.0114em for 14px, `tracking.none` above, declared on every type role |
| E40 | [Material Design 3 — Text fields specs](https://m3.material.io/components/text-fields/specs) | 2026-09-04 | S | The outlined text field's colour roles: `Outline` for the enabled container outline and `Primary` for the focused one; the error-state figures draw the outline in `Error`, which the page's text lists for no other component part | ADOPT as the `accent` colour kind: `primary` and `error` are fills that are also drawn as lines (`bg`, `border`, `outline` channels), held at 3:1 against `surface` and `surface-lowest` by COLOR_PAIRS; it does NOT prove the pairs clear, only that M3 draws them |
| E41 | [Carbon Design System — Color tokens](https://carbondesignsystem.com/elements/color/tokens/) | 2026-09-04 | S | White theme: `$focus` ("Focus border; Focus underline") is Blue 60 #0f62fe, the same value as `$interactive` and `$border-interactive`; `$focus-inset` ("Contrast border paired with $focus") is white, and `$focus-inverse` white "on high contrast moments" | ADAPT: the focus ring is declared as an alias of `primary` rather than a second copy of its primitive, and the separation Carbon gets from `$focus-inset` ours gets from `size.focus-offset` (2px of surface). REJECT for now: Carbon's dark theme flips `$focus` to white; teal.500 on ink.950 measures 6.2:1 so no flip is owed. It does NOT prove the offset ring is perceivable on a primary fill; nothing measures that yet |
