...
| [034](ADR-034-style-relationships-are-declared-never-inferred.md) | Xforge is a closed design language: every value, relationship and CSS capability is declared before use; one compiler; ADR-031 consumes the generated vocabulary | Proposed; all ten steps and the follow-through built | `tokens.test.ts` contract refusals and `design-system-classes.test.ts` closure checks (RED first, green 2026-09-03), `gen:tokens && git diff --exit-code` |
