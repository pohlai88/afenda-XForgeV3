...
| [031](ADR-031-component-policy-is-authored-the-primitive-is-projected.md) | Component policy is authored beside the component; the React primitive is hand-written; generation REJECTED; Adaptation Protocol (Beta) | Accepted (amended); Protocol creation core FROZEN; maintenance loop run once, RECONCILE provisional | `adapter-schema.test.ts` + seven component tests (RED first, green 2026-09-03); beta exit A yes, B half, C none found lexically, D yes; Switch/Combobox behaviour proved in Chromium (`vitest run --project browser`) |
| [032](ADR-032-an-agent-may-not-restore-a-deleted-file.md) | An agent may not restore a deleted file | FROZEN | review rule: `git log --diff-filter=D -- <path>` on any added file |
...
| [034](ADR-034-style-relationships-are-declared-never-inferred.md) | Xforge is a closed design language: every value, relationship and CSS capability is declared before use; one compiler; ADR-031 consumes the generated vocabulary | Proposed; all ten steps and the follow-through built | `tokens.test.ts` contract refusals and `design-system-classes.test.ts` closure checks (RED first, green 2026-09-03), `gen:tokens && git diff --exit-code`; no external run yet |
