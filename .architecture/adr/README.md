...
| [034](ADR-034-style-relationships-are-declared-never-inferred.md) | Xforge is a closed design language: every value, relationship and CSS capability is declared before use; one compiler; ADR-031 consumes the generated vocabulary | Proposed; nothing built | `tokens.test.ts` refusals (unwritten, RED first), `gen:tokens && git diff --exit-code` |
