...
| Material Design 3 *Text fields specs* — the outlined field's roles are `Outline` enabled and `Primary` focused, and its error-state figures draw the outline in `Error` (**E40**, read 2026-09-04) | Why `primary` and `error` are the `accent` kind: a fill that is also a line, so `STYLE.error.default.border` and `.outline` exist and the edge is held at 3:1 against the surfaces. ADAPT: M3 does not publish the pairs' ratios; `COLOR_PAIRS` measures them |
| Carbon *Color tokens* — `$focus` is Blue 60, the value of `$interactive`, with `$focus-inset` white as the "contrast border paired with $focus" (**E41**, read 2026-09-04) | Why `semantic.color.focus` is an alias of `primary` and not a second copy of teal.700: the coincidence is the design, declared once; the 2px offset does the inset's job. Not proved: that an offset ring reads on a primary fill |
...
The same pass, typography (E38, HEAD `37b389e`): the nine type roles placed against Material 3's
fifteen from Google's token file. Six carry a style to the pixel — caption/body-small,
